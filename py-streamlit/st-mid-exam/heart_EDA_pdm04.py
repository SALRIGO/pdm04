...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm04')
...
classes=df.target
noHB,HB=classes.value_counts()
st.sidebar.write('non-Heart-Disease(noHD):',noHB)
st.sidebar.write('Heart-Disease(HD):',HB)
...
# hist_cats = df['Outcomes'].sort_values().unique()
...
sns.heatmap(df[['age','cp','thalach','exang','oldpeak','slope','ca','thal']].corr(), annot=True, vmin=-1, vmax=1, cmap='coolwarm')
st.pyplot(fig6)
